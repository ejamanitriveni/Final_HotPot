...
namespace HotPot.Controllers
{
    ...
    public class CustomerController : ControllerBase
    {
        ...
        [Route("GetAllMenus")]
        [HttpGet]
        public async Task<ActionResult<List<Menu>>> GetAllMenus([FromQuery] string? searchQuery)
        {
            try
            {
        ...
        public async Task<ActionResult<List<Restaurant>>> GetAllRestaurants([FromQuery] string? searchQuery)
        {
            try
            {
                ...
                if (searchQuery != null && searchQuery != "")
                {
                    result = result.Where(r => r.RestaurantName.Contains(searchQuery) ||
                    r.Phone.Contains(searchQuery) ||
                    r.Email.Contains(searchQuery)
                    //r.CityId.(searchQuery) ||
                    ).ToList();
                }
                return result;
            }
            ...
            {
                _logger.LogCritical(e.Message);
                return NotFound("No Restaurant available to show at the moment");
            }
        }
